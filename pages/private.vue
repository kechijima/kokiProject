<script setup>
import { getPrivateKamokuRows } from '@/composables/useSheet';
...
onMounted(async () => {
    await fetchData();
    initializeLiff('2005378903-OgqmNgzY');
});

async function fetchData() {
    try {
        const { data, error } = await getPrivateKamokuRows();
